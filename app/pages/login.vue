...
useHead({ title: 'Login | SIMRS Enterprise EMR' })
...
<template>
  <main class="min-h-screen flex items-center justify-center bg-gray-50 relative overflow-hidden">
    ...
    <div class="absolute -top-40 -left-40 w-96 h-96 bg-blue-100 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob"/>
    <div class="absolute top-40 -right-40 w-96 h-96 bg-indigo-100 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-2000"/>
    <div class="absolute -bottom-40 left-20 w-96 h-96 bg-blue-50 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-4000"/>

    <div class="relative w-full max-w-5xl flex rounded-2xl shadow-2xl bg-white overflow-hidden z-10 border border-gray-100 mx-4">
      
      <!-- Left Panel: Illustration / Branding -->
      <div class="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-blue-600 to-indigo-800 p-12 text-white flex-col justify-between relative">
        ...
        <div class="relative z-10">
          <div class="flex items-center gap-3 mb-8">
            ...
            <span class="text-2xl font-bold tracking-wider">SIPANTES</span>
          </div>
          
          <h1 class="text-4xl font-extrabold leading-tight mb-4">
            SIMRS<br>Enterprise EMR
          </h1>
          <p class="text-blue-100 text-lg max-w-sm leading-relaxed">
            Sistem Informasi Manajemen Rumah Sakit Terintegrasi. Mengelola IGD, Rawat Jalan, dan Rawat Inap secara komprehensif.
          </p>
        </div>

        <div class="relative z-10 flex items-center gap-4 text-sm font-medium text-blue-200">
          <div class="flex -space-x-2">
            <div class="w-8 h-8 rounded-full bg-blue-500 border-2 border-blue-700 flex items-center justify-center text-xs">IGD</div>
            <div class="w-8 h-8 rounded-full bg-indigo-500 border-2 border-indigo-700 flex items-center justify-center text-xs">RJ</div>
            <div class="w-8 h-8 rounded-full bg-purple-500 border-2 border-purple-700 flex items-center justify-center text-xs">RI</div>
          </div>
      ...
      <div class="w-full lg:w-1/2 p-8 sm:p-12 md:p-16 flex flex-col justify-center bg-white relative">
        <div class="max-w-md w-full mx-auto">
          ...
          <div class="lg:hidden flex items-center gap-2 mb-8">
            ...
            <span class="text-xl font-bold text-gray-900">SIMRS Enterprise</span>
          </div>
          ...
          <form class="space-y-5" @submit.prevent="submit">
            <!-- Alert Error -->
            <div v-if="error" class="bg-red-50 border-l-4 border-red-500 p-4 rounded-md flex items-start gap-3 animate-fade-in">
            ...
            >
              <svg v-if="auth.loading" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
              </svg>
              {{ auth.loading ? 'Memverifikasi...' : 'Masuk ke Sistem' }}
            </button>
          </form>

          <div class="mt-8 pt-6 border-t border-gray-100">
            <p class="text-center text-xs text-gray-400">
              &copy; {{ new Date().getFullYear() }} RSUD Otista Kabupaten Bandung. Hak Cipta Dilindungi.<br>
              Aplikasi Rebranding v1.0 (SIPANTES)
            </p>
          </div>
        </div>
      </div>
